...
remainingload=sum(busdata(:,5));

[jkl,kjl]=size(nl);
[p,q]=size(nr);
for klo=1:jkl
    V_sending=busdata((nl(klo,1)),3);
    V_receiving=busdata((nr(klo,1)),3);
    delta_km=(busdata((nl(klo,1)),4))-(busdata((nr(klo,1)),4)); %
    w=((2*V_receiving./V_sending).*cosd((delta_km)))-1;
    li(klo)=w.*w;
end
LI=transpose(li);%loading indices of every branch
xs=[nl nr LI];
